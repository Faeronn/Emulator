				...
				if(msg.charAt(0) == '.')
				{
					...
					if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("infos"))
					{
						...
						String mess =	"===========\n"
							...
							+			"Joueur(s) en ligne(s): "+Ancestra.gameServer.getPlayerNumber()+"\n"
							+			"Record de connexions: "+Ancestra.gameServer.getMaxPlayer()+"\n"
							+			"<i>Compilé par </i><b>NeOToXiC</b><i> d'aidemu.</i>\n"
							+			"===========\n";
					...
					if(msg.length() > 5 && msg.substring(1, 6).equalsIgnoreCase("fmcac")) //Adlesne Commande
	                    {
	                            ...
	                            {
	                                    SocketManager.GAME_SEND_MESSAGE(_perso,  "Action impossible : vous avez moins de 500.000 k", Ancestra.CONFIG_MOTD_COLOR);
	                                    return;
	                            }
						...
						if(msg.length() > 6 && msg.substring(1, 7).equalsIgnoreCase("parcho")) //Commande .parcho
					      {
					        ...
					        if(nbreElement == 0)
					        {
					            SocketManager.GAME_SEND_Im_PACKET(_perso, "116;<i>System</i>~Vous avez déjà plus de 100 partout !");
					        }
					        else
					        {
					            SocketManager.GAME_SEND_STATS_PACKET(_perso);
					            SocketManager.GAME_SEND_Im_PACKET(_perso, "116;<i>System</i>~Vous êtes parcho 101 en " + element + " !");
					        }
					        return;
					      }
